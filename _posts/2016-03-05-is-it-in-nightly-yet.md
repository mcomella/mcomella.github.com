...
### React & writing robust code

I've been thinking a lot about ways to construct more robust applications
in less time. To this end, [React][react] comes up a lot and this seemed
a great opportunity to finally get my toes wet. Here are some things I
liked:
...
The initial implementation was slow when a changeset was not in Nightly
because the server didn't return fast results for invalid changeset IDs.
I decided to add a fast fail implementation to `mozhg.isRevisionSetValid`,
which is when I came to benefit from it. For this method, I initially
wrote some tests that passed the first time -- you know, the kind that
you have to intentionally break to make sure they're actually working
-- and, given how simple the code I added was<sup>3</sup>, I was surprised
to find the tests failing, despite the application seeming to perform
correctly with manual testing.
